...
import { sendSupportTicketEmail } from "@/lib/email/tickets"
import { createClient, createRLSClient } from "@/lib/supabase/server"
import { supportTicketSchema } from "@/lib/validations/tickets"
...
async function getAttachmentFileBase64(formData: FormData): Promise<{
  ...
  // Keep attachments small to avoid huge emails / memory usage.
...
export async function POST(request: NextRequest) {
  try {
    ...
    const raw = {
      category: getString(formData, "category").trim(),
      description: getString(formData, "description").trim(),
      steps: getString(formData, "steps").trim() || undefined,
      email: contactEmail,
    ...
    if (insertError || !inserted) {
      ...
      if (process.env.NODE_ENV !== "production") {
        return errorResponse(insertError?.message ?? "Failed to create support ticket", 500)
      }
    ...
    return handleUnexpectedError(error, "POST /api/tickets/support")
  }
}
